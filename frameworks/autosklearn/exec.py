...
def run(dataset, config):
    ...
    metrics_mapping = dict(
        ...
        if askl_version < version.parse("0.10")
    ...
    if ml_memory_limit == "auto":
        ml_memory_limit = max(
            min(config.max_mem_size_mb / n_jobs, math.ceil(total_memory_mb / n_jobs)),
            3072,  # 3072 is autosklearn default and we use it as a lower bound
        )
    if isinstance(askl_version, version.LegacyVersion) or askl_version >= version.parse(
        "0.11"
    ):
        log.info(
        ...
        if ensemble_memory_limit == "auto":
            ensemble_memory_limit = max(
                math.ceil(ml_memory_limit - (total_memory_mb - config.max_mem_size_mb)),
                math.ceil(ml_memory_limit / 3),  # default proportions
                1024,
            )  # 1024 is autosklearn defaults
        log.info(
    ...
    def infer(data: Union[str, pd.DataFrame]):
        ...
        predict_fn = (
            auto_sklearn.predict_proba if is_classification else auto_sklearn.predict
        )
    ...
    if config.measure_inference_time:
        ...
        inference_times["df"] = measure_inference_times(
            infer,
            [(1, sample_one_test_row(seed=i)) for i in range(100)],
        )
